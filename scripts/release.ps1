...
.DESCRIPTION
  This script updates the package version in `pyproject.toml` and `fabricla_connector/__init__.py`,
...
.USAGE
  # Bump to 1.0.1, commit and tag locally
  .\release.ps1 -NewVersion 1.0.1

  # Bump, commit, tag and push to origin
  .\release.ps1 -NewVersion 1.0.1 -Push

  # Bump, commit, tag, push and run build_pypi.ps1
  .\release.ps1 -NewVersion 1.0.1 -Push -Build
...
$initFile = Join-Path $repoRoot 'src\fabricla_connector\__init__.py'
...
Write-Info "Updating fabricla_connector/__init__.py..."
...
if (-not $found) {
    ...
    $lines = $lines[0..($insertIdx-1)] + ("__version__ = `"$NewVersion`"") + $lines[$insertIdx..($lines.Count-1)]
}
